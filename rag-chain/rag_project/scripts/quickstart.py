...
def main() -> None:
    ...
    docs = load_all_pdfs()
    if not docs:
        console.print("[red]ERROR data/sample_docs/ 下没有 PDF,请先运行:python scripts/generate_synthetic_data.py[/red]")
        sys.exit(1)
    ...
    if not contexts:
        console.print("[yellow]WARNING 没检索到相关内容,换个更具体的问法试试。[/yellow]")
        sys.exit(0)
